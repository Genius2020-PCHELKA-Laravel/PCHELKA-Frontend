...
import CurtainCleaningDetails from '../components/CurtainCleaningSteps/CurtainCleaningDetails';
import DateandTimeDetails from '../components/CurtainCleaningSteps/DateandTimeDetails';
import AddressDetails from '../components/CurtainCleaningSteps/AddressDetails';
import Payment from '../components/CurtainCleaningSteps/Payment';
import BookedScreen from '../components/CurtainCleaningSteps/BookedScreen';

import Spacer from '../components/Spacer';
import { TouchableOpacity } from 'react-native-gesture-handler';
import { AntDesign, FontAwesome5 } from '@expo/vector-icons';
import ModalDetails from '../components/CurtainCleaningSteps/ModalDetails';
...
const CurtainCleaningScreen = ({ navigation, t }) => {
  ...
  useEffect(() => {
    console.log("CurtainCleaningScreen::UseEffect::gethourprice");
    console.log("CurtainCleaningScreen::UseEffect::State hour price:: " + hcstate.CA.name)

    console.log("CurtainCleaningScreen::UseEffect::gethourprice");
    console.log("CurtainCleaningScreen::UseEffect::State hour price:: " + hcstate.CA.hourPrice)

    console.log("CurtainCleaningScreen::UseEffect::gethourmaterialprice");
    console.log("CurtainCleaningScreen::UseEffect::State hour material price:: " + hcstate.CA.materialPrice);
    getProviders({ serviceType: 'CurtainCleaning' }).then((response) => {
      console.log("CurtainCleaniningScreen::Providers");
      console.log(response);
    }).catch((error) => {
      console.log("Error::CurtainCleaniningScreen::Providers");
      console.log(error);
    });
  ...
  const onSubmitSteps = async () => {
    ...
    if (hcstate.method == 0 && hcstate.valid == true) {
      ...
      var result = await pay({
        ...
        console.log("CurtainCleaningScreen::paid");
        ...
        console.log("CurtainCleaningScreen::NOTpaid" + error);
        setIsLoading(false);
        ispaid = 'no';
    ...
    if (hcstate.method == 1 || ispaid == 'yes') {
      ...
      var quantity = -1;
      if (hcstate.quantity == 2) quantity = 115;
      if (hcstate.quantity == 3) quantity = 116;
      if (hcstate.quantity == 4) quantity = 117;
      if (hcstate.quantity == 5) quantity = 118;
      if (hcstate.quantity == 6) quantity = 119;
      if (hcstate.quantity == 7) quantity = 120;
      if (hcstate.quantity == 8) quantity = 121;
      if (hcstate.quantity == 9) quantity = 122;
      if (hcstate.quantity == 10) quantity = 123;
      if (hcstate.quantity == 11) quantity = 124;
      if (hcstate.quantity == 12) quantity = 125;
      ...
      HCBooking({
        ...
        frequency: frequency,
        materialPrice: hcstate.squaremeters * hcstate.materials * hcstate.CU.materialPrice,
        answers: [
          {
            questionId: 29,
            answerId: null,
        ...
        getUpcoming().then((response) => {
          ...
          console.log("CutrainCleaningScreen::afterBooking::getUpcoming::response:: ");
          //console.log("######################" + JSON.stringify(response));
          hcdispatch({
            type: 'RESET'
          });
